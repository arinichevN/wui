function AoidCommandDetector(commands){
	...
	this.commandSupported = function(v){
		...
		if(v !== null){
			...
			if(data instanceof Array && data.length == 1){
				...
				let supp = parseInt(data[0].v4);
				if(!(isNaN(app_id) || isNaN(obj_id) || isNaN(cmd_id) || isNaN(supp))){
					if(app_id === this.app_id && obj_id === this.obj_id && cmd_id === command_id){
						supported = supp;
					}
				}
			}
	...
	this.sendRequest = function(command) {
		let d = acp_buildRequest([ACPP_SIGN_REQUEST_GET, CMD_AOID_GET_ACP_COMMAND_SUPPORTED, this.app_id, this.obj_id, command]);
