function AoidParamArrInt(master, id, parent_id, kind, description_id, length, minv, maxv) {
	this.master = master;
	this.id = id;
		...
		if(supported){
			...
			switch(command){
				...
				case CMD_AOID_GET_NVRAM_VALUE:
					new_item = new AoidCommandGetArrInt(this, command, this.length);
					break;
				case CMD_AOID_SET_NVRAM_VALUE:
					new_item = new AoidCommandSetArrInt(this, command, this.length, this.minv, this.maxv);
					break;
			}
