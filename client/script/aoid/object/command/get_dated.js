function AoidCommandGetDateD(master, command){
	this.master = master;
	...
	this.updateContent = function(v){
			...
			if(data instanceof Array && data.length == 1){
				...
				if(!(isNaN(app_id) || isNaN(id) || isNaN(val) || isNaN(success))){
					if(app_id == this.master.master.app_id && id == this.master.id){
						if(success == YES){
							let y = val / DAYS_IN_YEAR;
							let m = (val % DAYS_IN_YEAR) / DAYS_IN_MONTH;
							let d = val % DAYS_IN_MONTH;
							elemvstr = intTo2str(Math.floor(y)) + "-" + intTo2str(Math.floor(m)) + "-" + intTo2str(d);
							elemv = val;
						}
					}
				}
		...
		this.value = elemv;
		if(elemv !== null){
			updateStrElemGood(this.container, elemvstr);
		}else{
