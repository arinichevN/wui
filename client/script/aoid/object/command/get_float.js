function AoidCommandGetFloat(master, command, precision){
	this.master = master;
	...
	this.updateContent = function(v){
		...
		if(v !== null){
			...
			if(data instanceof Array && data.length == 1){
				...
				let val = parseFloat(data[0].v3);
				let success = parseInt(data[0].v4);
				if(!(isNaN(app_id) || isNaN(id) || isNaN(val) || !isFinite(val) || isNaN(success))){
		...
		this.value = elemv;
		if(elemv !== null){
			updateStrElemGood(this.container, elemv.toFixed(this.precision));
		}else{
